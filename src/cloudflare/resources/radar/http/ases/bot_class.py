...
class BotClassResource(SyncAPIResource):
    ...
    *,
    ...
    timeout: float | httpx.Timeout | None | NotGiven = NOT_GIVEN,) -> BotClassGetResponse:
        """Get the top autonomous systems (AS), by HTTP traffic, of the requested bot
        class.

        These two categories use Cloudflare's bot score - refer to
        ...
        if not bot_class:
          raise ValueError(
            f'Expected a non-empty value for `bot_class` but received {bot_class!r}'
          )
        return self._get(
            ...
            }, bot_class_get_params.BotClassGetParams), post_parser=ResultWrapper[BotClassGetResponse]._unwrapper),
