...
class DeviceTypeResource(SyncAPIResource):
    ...
    *,
    ...
    timeout: float | httpx.Timeout | None | NotGiven = NOT_GIVEN,) -> DeviceTypeGetResponse:
        """Get the top locations, by HTTP traffic, of the requested device type.
        ...
        if not device_type:
          raise ValueError(
            f'Expected a non-empty value for `device_type` but received {device_type!r}'
          )
        return self._get(
            ...
            }, device_type_get_params.DeviceTypeGetParams), post_parser=ResultWrapper[DeviceTypeGetResponse]._unwrapper),
