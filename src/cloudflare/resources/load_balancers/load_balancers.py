...
class LoadBalancers(SyncAPIResource):
    ...
    def create(
        ...
        *,
        ...
        return self._post(
            f"/zones/{identifier}/load_balancers",
    ...
    def update(
        ...
        *,
        ...
        """
        if not identifier1:
            raise ValueError(f"Expected a non-empty value for `identifier1` but received {identifier1!r}")
        if not identifier:
            raise ValueError(f"Expected a non-empty value for `identifier` but received {identifier!r}")
        return self._put(
            f"/zones/{identifier1}/load_balancers/{identifier}",
            body=maybe_transform(
                {
                    "default_pools": default_pools,
                    "fallback_pool": fallback_pool,
