...
class ByTagResource(SyncAPIResource):
    ...
    *,
    ...
    timeout: float | httpx.Timeout | None | NotGiven = NOT_GIVEN,) -> ByTagGetResponse:
        """
        ...
        if not rule_tag:
          raise ValueError(
            f'Expected a non-empty value for `rule_tag` but received {rule_tag!r}'
          )
        return self._get(
            f"/accounts/{account_id}/rulesets/{ruleset_id}/versions/{ruleset_version}/by_tag/{rule_tag}",
            options=make_request_options(extra_headers=extra_headers, extra_query=extra_query, extra_body=extra_body, timeout=timeout, post_parser=ResultWrapper[ByTagGetResponse]._unwrapper),
